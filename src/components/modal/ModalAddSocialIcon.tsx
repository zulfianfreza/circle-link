...
export default function ModalAddSocialIcon({
  ...
  const [listIcon, setListIcon] = useState(ICON)
  ...
  const handleSelectSocial = (social: string) => {
    setStep(step + 1)
          ...
          {listIcon.map((icon, index) => (
            <IconItem
              ...
              disabled={socialIcons?.some(
                (socialIcon) => socialIcon.social == icon.label
              )}
              onClick={() => handleSelectSocial(icon.label)}
            />
          ))}
        </div>
      </div>
  ...
  if (step === STEP.ADD) {
    dialogBody = (
      <div className=' px-5 pb-5'>
        <Input
          label={`Enter ${selectedSocial} URL`}
          value={urlSocial}
                ...
                <div className=' relative p-4 py-6'>
                  ...
                  <div className=' text-center'>
                    <p className=' font-semibold'>
                      Add {step === STEP.ADD ? selectedSocial : null} Icon
                    </p>
                  </div>
                  <div
